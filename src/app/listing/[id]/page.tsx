...
export default async function Listing({ params }: ListingParams) {
  ...
  try {
    const result = await fetch(process.env.URL + "/api/listing/get", {
      ...
      body: JSON.stringify({ listingId: id }), // Using awaited id here
      cache: "no-store",
    });
    const data = await result.json();
    listing = data[0];
  } catch (error) {
    listing = { name: "Failed to load listing" } as Listing;
  }
